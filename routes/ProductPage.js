...
router.post("/addToCart", (req, res) => {
  ...
  User.findOne({ _id: req.body.userID }, { cart: 1, _id: 0 })
    .then((response) => {
      var hasEntry = false;
    ...
    .then((response) => {
      ...
      User.updateOne(
        ...
        .then((response) => {
          res
            .status(200)
            .json({ messag: "product Successfully Deducted from users cart" });
        })
        .catch((err) => {
          res.status(300).json({ message: "Something Went Wrong...!" });
        });
      ...
      if (result[0].wishlist.includes(req.body.productID)) {
        ...
        User.updateOne(
          ...
          .then((response) => {
            res
              .status(200)
              .json({ message: "product Added to the wish list " });
          })
          .catch((err) => {
      ...
      User.updateOne(
        ...
        .then((answer) => {
          ...
          req.body.itemList.map((item) => {
            Product.findOne({ _id: item.productID }, { costofitem: 1, _id: 0 })
              .then((result) => {
                if (result.costofitem > item.count) {
                  var newCount =
                    parseInt(result.costofitem) - parseInt(item.count);
                  product
                    .updateOne(
                      { _id: item.productID },
                      {
