...
router.post("/userMessage", (req, res) => {
  ...
  newMail
    ...
    .catch((err) => {
      res
        .status(300)
        .json({ message: "Something went wrong in sending message" });
    });
});
  ...
  jwt.verify(req.token, "secretkey", (err, result) => {
    Contact.find()
      ...
      .catch((err) => {
        res
          .status(300)
          .json({ message: "something went Wriong in fetching mails." });
      });
  });
});
...
router.post("/getMail", verifyToken, (req, res) => {
  jwt.verify(req.token, "secretkey", (err, result) => {
    Contact.findOne({ _id: req.body.selectedConversationID })
      .then((response) => {
        res.status(200).json({ response });
      })
      .catch((err) => {
...
router.post("/replyByAdmin", verifyToken, (req, res) => {
  jwt.verify(req.token, "secretkey", (err, result) => {
    Contact.updateOne(
      { _id: req.body.selectedConversationID },
      {
        $set: {
...
Thank you, Regards.
      ...
      .catch((err) => {
        console.log(err, "err");
        res.status(300).json({
          message: "Something Went Wroing in replying the message..!",
        });
      });
  });
});
