    ...
    .then(async (response) => {
      ...
      if (validPasswrod) {
        if (req.body.email === response[0].email) {
          ...
          res.status(300).json({ message: "Enter Correct Email Address" });
        }
      } else {
        res.status(300).json({ message: "Wrong Password. Enter Again!" });
      }
    })
    .catch((err) => {
      console.log(err);
      res.status(300).json({ message: "Enter Valid Credentials", err });
    });
});
